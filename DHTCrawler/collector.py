    ...
    def _handle_alerts(self, session, alerts):
        while len(alerts):
            ...
            if isinstance(alert, lt.add_torrent_alert):
                ...
                info_hash = alert.info_hash.to_string().encode('hex')#if 收到 announce,記錄下hash值
                ...
                info_hash = alert.info_hash.to_string().encode('hex') #if 收到 get peer，記錄下hash值
                if info_hash in self._meta_list:
                    self._meta_list[info_hash] += 1
                else:
        ...
        while True:
            ...
            try:
                ...
                with open(self._result_file, 'wb') as f:
                    json.dump(self._meta_list, f,indent=1)
            except Exception as err:
                pass
